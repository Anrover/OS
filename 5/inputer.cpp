...
void skeleton_daemon() {
    ...
    //перенаправляем весь вывод в /dev/null
    int devNull = open("/dev/null", O_WRONLY);
    if(devNull == -1){
        syslog(LOG_ERR, "Error in open('/dev/null',0)");
        exit(EXIT_FAILURE);
    }
    if(dup2(devNull, STDOUT_FILENO) == -1) {
        syslog(LOG_ERR, "Error in dup2(devNull, STDOUT_FILENO)");
        exit(EXIT_FAILURE);
    }

	// Пришлось перенаправлять вывод, потому как если его просто закрыть, то, например, 
	// вызов system("/bin/ls -l") возвращал ненулевой код ошибки
    //close(STDOUT_FILENO); 
    close(STDIN_FILENO);
    close(STDERR_FILENO);
}
